...
DMA_HandleTypeDef hdma_adc1; // DMA 핸들러 정의 (ADC와 DMA 연동을 위한 핸들러)
...
void adc_pa0_interrupt_init(void)
{
    ...
    // ADC 인터럽트 설정
    NVIC_SetPriority(ADC_IRQn, 0);  // 인터럽트 우선순위 설정
...
void adc_pa0_dma_init(void)
{
    ...
    hadc1.Init.DataAlign = ADC_DATAALIGN_RIGHT; // 데이터 정렬: 오른쪽 정렬
    hadc1.Init.NbrOfConversion = 1; // 한 채널만 변환
    hadc1.Init.DMAContinuousRequests = ENABLE; // DMA 연속 요청 활성화
    hadc1.Init.EOCSelection = ADC_EOC_SINGLE_CONV; // 단일 변환 완료 시 종료
