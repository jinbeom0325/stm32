...
void gpio_pc13_interrupt_init(void)
{
    ...
    //PA0
    GPIO_InitStruct.Pin = GPIO_PIN_0;           // 핀 번호 설정
	GPIO_InitStruct.Mode = GPIO_MODE_IT_RISING;  // 상승엣지(버튼 눌릴 때) 인터럽트
	GPIO_InitStruct.Pull = GPIO_NOPULL;          // 풀업/풀다운 없음
	GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW; // 속도는 낮음
    ...
    HAL_NVIC_SetPriority(EXTI0_IRQn, 0, 0);   // 우선순위 0 (가장 높음)
    HAL_NVIC_EnableIRQ(EXTI0_IRQn);          // EXTI15_10 인터럽트 활성화
...
{
    HAL_GPIO_EXTI_IRQHandler(GPIO_PIN_0);       // HAL이 제공하는 인터럽트 핸들러 호출
}
